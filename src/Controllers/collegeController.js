...
const createCollege = async (req, res) => {
  try {
    ...
    if (collegeName) {return res.status(400) .send({ status: false, message: "college detail already exists",})
    }
   ...
   const requiredData = await collegeModel.findById(college._id).select({ _id: 0, __v: 0 });
